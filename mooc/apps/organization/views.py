    ...
    def get(self, request, org_id):
        ...
        if request.user.is_authenticated:
            if UserFavorite.objects.filter(user=request.user, fav_id=int(course_org.id), fav_type=2):
                has_fav = True
        #判断是否收藏
        all_courses = course_org.course_set.all()[:3]  #通过机构的信息外键来取出对应的所有课程
...
class OrgTeachersView(View):
    def get(self, request, org_id):
        ...
        for name in teachers_org:
            kctj = Course.objects.filter(kcjs_org=name)
            kctj2 = len(kctj)
...
class TeacherDetailViews(View):
    def get(self, request, teacher_id):
        ...
        teacher_detail.save()
        all_courses = Course.objects.filter(kcjs_org=teacher_detail) #取出教师的课程信息

...
class IndexView(View):
    def get(self, request):
        banners = Banner.objects.all().order_by("index")[:8] #首页的轮播图
        course_tg = Course.objects.filter(is_tg=True).order_by("-id")[:5] #首页课程的轮播图
        course = Course.objects.filter(is_tg=False).order_by("-id")[:6] #获取首页全部课程，去除推广的课程
...
class MyFavCourseView(LoginRequiredMIxin, View):
    def get(self, request):
        ...
        for uif in user_course:
            uid = uif.fav_id
            user_course = Course.objects.get(id=int(uid))
